...
{
	//if (!_WasUsed) //?throw runtime_error("ObjectTemplateProxy::UnregisterNamedPropertyHandlers(): You cannot change the object template named handlers after an object was already created from it.");
...
{
	//if (!_WasUsed) //?throw runtime_error("ObjectTemplateProxy::UnregisterIndexedPropertyHandlers(): You cannot change the object template index handlers after an object was already created from it.");
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->NamedPropertyGetter != nullptr)
		{
			if (proxy != nullptr && proxy->_EngineProxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				auto hNameStr = hName->IsSymbol() ? hName.As<Symbol>()->Description(engine->Isolate()).As<String>() : hName.As<String>();
				...
				try {
					...
					ThrowException(NewString("'NamedPropertyGetter' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); 
				}
				engine->_InCallbackScope--;
				str.Dispose();
				if (result != nullptr)
				{
					if (result->IsError())
						info.GetReturnValue().Set(ThrowException(Exception::Error(result->Handle()->ToString(engine->Context()).FromMaybe(Local<String>()))));
					else
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->NamedPropertySetter != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'NamedPropertySetter' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->NamedPropertyQuery != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'NamedPropertyQuery' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				str.Dispose();
				if (result >= 0)
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->NamedPropertyDeleter != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'NamedPropertyDeleter' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				str.Dispose();
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->NamedPropertyEnumerator != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'NamedPropertyEnumerator' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				if (result != nullptr)
				{
					if (result->IsError())
					{
						...
						array->Set(engine->Context(), 0, ThrowException(Exception::Error(result->Handle()->ToString(engine->Context()).FromMaybe(Local<String>()))));
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->IndexedPropertyGetter != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'IndexedPropertyGetter' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				if (result != nullptr)
				{
					if (result->IsError())
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->IndexedPropertySetter != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'IndexedPropertySetter' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				engine->ProcessHandleQueues(); // (since setting properties may dispose another, do this at least once)
				if (result != nullptr)
				{
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->IndexedPropertyQuery != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'IndexedPropertyQuery' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				if (result >= 0)
					info.GetReturnValue().Set(Handle<v8::Integer>(NewInteger(result)));
			}
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->IndexedPropertyDeleter != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'IndexedPropertyDeleter' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
	...
	if (obj->InternalFieldCount() > 1)
	{
		...
		if (!field->IsUndefined() && proxy->IndexedPropertyEnumerator != nullptr)
		{
			if (proxy != nullptr && proxy->Type == ObjectTemplateProxyClass)
			{
				...
				catch (...) { ThrowException(NewString("'IndexedPropertyEnumerator' no longer exists - perhaps the GC collected it.").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				if (result != nullptr)
				{
					if (result->IsError())
...
HandleProxy* ObjectTemplateProxy::CreateObject(int32_t managedObjectID)
{
	...
	_WasUsed = true;
	auto proxyVal = _EngineProxy->GetHandleProxy(ToLocalThrow(_ObjectTemplate->NewInstance(_EngineProxy->Context())));
...
void ObjectTemplateProxy::AccessorGetterCallbackProxy(Local<Name> property, const PropertyCallbackInfo<Value>& info)
{
	...
	if (!obj.IsEmpty())
	{
		...
		if (hAccessors->Length() == 3)
		{
			...
			auto hID = ToLocalThrow(hAccessors->Get(engine->Context(), 0)).As<Integer>(); // ManagedObjectID
			auto managedObjectID = (int32_t)ToThrow(hID->Int32Value(ctx));
			...
			auto getter = (ManagedAccessorGetter)ToLocalThrow(hAccessors->Get(engine->Context(), 1)).As<External>()->Value();
			...
			if (getter != nullptr)
			{
				...
				auto str = engine->GetNativeString(*ToLocalThrow(property->ToString(engine->Context())));
				...
				catch (...) { ThrowException(NewString("'AccessorGetterCallbackProxy' caused an error - perhaps the GC collected the delegate?").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
				...
				{
					if (result->IsError())
						hResult = ThrowException(Exception::Error(result->Handle()->ToString(engine->Context()).FromMaybe(Local<String>()))); // TODO: Look into associating the returned error type as well (very low priority)
					else
...
void ObjectTemplateProxy::AccessorSetterCallbackProxy(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& info)
{
	...
	if (!obj.IsEmpty())
	{
		...
		if (hAccessors->Length() == 3)
		{
			...
			auto setter = (ManagedAccessorSetter)ToLocalThrow(hAccessors->Get(engine->Context(), 2)).As<External>()->Value();
			...
			if (setter != nullptr)
			{
				...
				catch (...) { ThrowException(NewString("'AccessorSetterCallbackProxy' caused an error - perhaps the GC collected the delegate?").FromMaybe(Local<String>())); }
				engine->_InCallbackScope--;
	...
	Handle<Value> hUndefined;
	ret.Set(hUndefined); //ret.SetUndefined();
}
	...
	v8::AccessControl access, v8::PropertyAttribute attributes)
{
	...
	_ObjectTemplate->SetAccessor(ToLocalThrow(NewUString(name)), AccessorGetterCallbackProxy, AccessorSetterCallbackProxy, accessors, access, attributes);  // TODO: Check how this affects objects created from templates!
}

void ObjectTemplateProxy::Set(const uint16_t *name, HandleProxy *value, v8::PropertyAttribute attributes)
{
	if (value != nullptr) {
		_ObjectTemplate->Set(ToLocalThrow(NewUString(name)), value->Handle(), attributes);  // TODO: Check how this affects objects created from templates!
	}
}
